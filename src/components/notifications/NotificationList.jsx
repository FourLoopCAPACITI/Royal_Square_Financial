...
export default function NotificationList({ items, onNavigate, allowComplete = false }) {
  ...
  return (
    <>
      {error && <p role="alert" className="p-4 text-brand-red">{error}</p>}
      <ul className="divide-y divide-brand-border">
        {items.map((item) => {
          const Icon = ICONS[item.kind];
          return (
            <li key={item.id} className={`flex gap-3 p-4 ${item.read ? '' : 'bg-brand-light-grey'}`}>
              <Icon size={19} className={`mt-1 shrink-0 ${item.urgent ? 'text-brand-red' : 'text-brand-grey'}`} aria-hidden="true" />
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 text-[12px] font-semibold">
                  <span className={item.urgent ? 'text-brand-red' : 'text-brand-grey'}>{item.label}</span>
                ...
                {item.date && <p className="mt-1 text-xs text-brand-grey">{item.dateLabel} <time dateTime={item.date}>{formatDate(item.date)}</time></p>}
