  ...
  return (
    <div className={`rounded-md border border-brand-border ${compact ? 'p-4' : 'p-5'}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-display text-[18px] font-medium">{goal.name}</p>
          <p className="flex items-center gap-1.5 text-[14.5px] text-brand-grey">
      ...
      {!compact && goal.targetDate && <p className="text-[14.5px] text-brand-grey">Target date {formatDate(goal.targetDate)}</p>}
    </div>
  );
}
