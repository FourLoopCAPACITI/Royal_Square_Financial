...
export default function NetWorthSummary({ assets = 0, liabilities = 0 }) {
  ...
  return (
    <div className="rounded-md border border-brand-border p-5">
      <p className="text-[13px] text-brand-grey">Estimated net worth</p>
      <p className="mt-1 font-display text-[30px] font-light tabular-nums leading-none">{formatZAR(netWorth)}</p>
      <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-brand-border pt-3 text-[14px]">
        <div>
          <dt className="text-brand-grey">Assets</dt>
          <dd className="font-semibold tabular-nums">{formatZAR(assets)}</dd>
        </div>
        <div>
          <dt className="text-brand-grey">Liabilities</dt>
          <dd className="font-semibold tabular-nums">{formatZAR(liabilities)}</dd>
        </div>
      </dl>
      <p className="mt-3 text-[12.5px] text-brand-grey">Based on information you've shared with your adviser.</p>
    </div>
  );
}
