...
export default function PhotoCapture({ kind, photos, onChange, tips }) {
  ...
  return (
    <div>
      <ul className="mb-4 list-disc space-y-1 pl-5 text-[16px] text-text-secondary">
      ...
      <p className="mt-2 text-[14px] text-brand-grey">{t('accident.photo.simulated')}</p>
      {photos.length > 0 && (
        <ul className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-4">
          {photos.map((p, i) => (
            <li key={p.id} className="relative flex aspect-square flex-col items-center justify-center rounded border border-brand-border bg-brand-light-grey text-brand-grey">
              ...
              <span className="mt-1 text-[13px]">{t('accident.photo.n', { n: i + 1 })}</span>
              <button type="button" onClick={() => onChange(photos.filter((x) => x.id !== p.id))} className="absolute right-1 top-1 rounded bg-surface p-0.5 hover:text-brand-red" aria-label={t('accident.photo.remove', { n: i + 1 })}>
                <X size={14} />
              </button>
            </li>
          ))}
