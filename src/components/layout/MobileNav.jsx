    ...
    <>
      ...
      <nav aria-label="Main" className="fixed inset-x-0 bottom-0 z-30 border-t border-brand-border bg-white pb-[env(safe-area-inset-bottom)] lg:hidden">
        <ul className="grid grid-cols-5">
          {primary.map(({ to, label, icon: Icon, end }) => (
            <li key={to}>
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex flex-col items-center gap-0.5 px-1 py-2 text-[12.5px] ${isActive ? 'font-semibold text-brand-red' : 'text-[#4A4A4A]'}`
                }
              >
          ...
          <li>
            <button type="button" onClick={() => setOpen(true)} className="flex w-full flex-col items-center gap-0.5 px-1 py-2 text-[12.5px] text-[#4A4A4A]" aria-expanded={open}>
