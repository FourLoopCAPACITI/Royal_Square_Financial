  ...
  return (
    <>
      ...
      <div className="mb-5 flex gap-2">
        {FILTERS.map((f) => (
          <button key={f.key} type="button" onClick={() => setFilter(f)} aria-pressed={filter.key === f.key} className={`rounded border px-3 py-1.5 text-[15.5px] font-semibold ${filter.key === f.key ? 'border-brand-red bg-brand-red text-white' : 'border-brand-border hover:border-brand-black'}`}>
            {f.label}
          </button>
        ))}
      </div>
