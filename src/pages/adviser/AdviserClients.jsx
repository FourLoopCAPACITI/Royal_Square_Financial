                  ...
                  <li key={c.id}>
                    <button type="button" onClick={() => setOpenId(open ? null : c.id)} aria-expanded={open} className="grid w-full gap-2 p-4 text-left hover:bg-brand-light-grey sm:grid-cols-[1.2fr_1fr_auto] sm:items-center">
                      <span>
                        ...
                        <span className="text-[15px] text-brand-grey">{c.occupation}. {c.city}</span>
                      </span>
                      <span className="text-[15.5px] tabular-nums text-brand-grey">Net worth {formatZAR(c.totalAssets - c.totalLiabilities)}</span>
                    ...
                    {open && (
                      <div className="space-y-3 border-t border-brand-border bg-brand-light-grey p-4">
                        ...
                        {mine.length ? mine.map((w) => <WorkflowCard key={w.id} workflow={w} providerName={providerName(w.providerId)} viewerRole="adviser" showOwner={false} />) : <p className="text-[15.5px]">No open processes.</p>}
                      </div>
                    )}
                  </li>
                );
